...
export default function SetupTriggerScreen({ route, navigation }) {
  const { name } = route.params || {};

  const handleSetTrigger = async () => {
    try {
      if (!ExpoHoldNative.hasDNDPermission()) {
        ExpoHoldNative.requestDNDPermission();
      }
    ...
    navigation.navigate('SetupConfirmation', { name });
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>
          We need "Do Not Disturb" and "Accessibility" permissions to block notifications and listen for volume buttons.
        </Text>
